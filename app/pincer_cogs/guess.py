...
from pincer.commands.components.decorators import button
...
class Guess:
    ...
    async def start(self, ctx: MessageContext):
        ...
        g = GuessingGame(self, ctx, channel, self.bot, ctx.author)
        await g.start()
    ...
    @button(style=ButtonStyle.DANGER, label="End Game")
    async def end_game_button(self, ctx):
